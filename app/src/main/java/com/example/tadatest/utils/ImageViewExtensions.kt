package com.example.tadatest.utils
...
fun ImageView.setImageFromUrl(imageUrl: String) {
    Glide.with(context).load(imageUrl).thumbnail(0.1f).into(this)
}

fun ImageView.setImageFromUrlWithProgressBar(url: String, progress: View) {
    Glide.with(this.context).load(url).thumbnail(0.1f).listener(PhotosRequestListener(progress))
        .into(this)
}

fun ImageView.setImageFromResourcesWithProgressBar(resourceId: Int, progress: View) {
    Glide.with(this.context).load(resourceId).thumbnail(0.1f)
